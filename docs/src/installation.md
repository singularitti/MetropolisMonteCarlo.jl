...
[`MetropolisMonteCarlo`](https://github.com/singularitti/MetropolisMonteCarlo.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("MetropolisMonteCarlo")
   ```

3. Run

   ```julia-repl
   julia> using MetropolisMonteCarlo
   ```
...
version of `MetropolisMonteCarlo`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/MetropolisMonteCarlo.jl"
```

in the second step above.

## Update `MetropolisMonteCarlo`
...
our [GitHub repository](https://github.com/singularitti/MetropolisMonteCarlo.jl)
for new releases.
Once we release a new version, you can update `MetropolisMonteCarlo` by typing

```@repl
using Pkg
Pkg.update("MetropolisMonteCarlo")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `MetropolisMonteCarlo`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("MetropolisMonteCarlo")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `MetropolisMonteCarlo`.
