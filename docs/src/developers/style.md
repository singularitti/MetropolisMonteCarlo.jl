...
!!! info
    The style guide is always a work in progress, and not all MetropolisMonteCarlo code
    follows the rules. When modifying MetropolisMonteCarlo, please fix the style violations
...
`MetropolisMonteCarlo` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/MetropolisMonteCarlo.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `MetropolisMonteCarlo` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `MetropolisMonteCarlo` have
